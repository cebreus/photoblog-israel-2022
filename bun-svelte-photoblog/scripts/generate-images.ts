...
function parseArgs(argv: string[]): ScriptArgs {
  const args: Omit<ScriptArgs, "concurrency"> & {
...
const log = {
  info: (...msg: any[]) =>
    !ARGS.quiet && console.log(pc.cyan("[images]"), ...msg),
  warn: (...msg: any[]) =>
    !ARGS.quiet && console.warn(pc.yellow("[images]"), ...msg),
  error: (...msg: any[]) => console.error(pc.red("[images]"), ...msg),
  verbose: (...msg: any[]) =>
    ARGS.verbose && !ARGS.quiet && console.log(pc.dim("[images]"), ...msg),
};
...
async function main() {
  ...
  if (ARGS.watch) {
    log.info(
      `Watch mode enabled. Watching ${toPosix(CTX.srcRoot)} and ${toPosix(CTX.contentRoot)}.`,
    );
...
async function runIncrementalBuild() {
  ...
  let cache = await loadJSON<Cache>(CTX.cachePath, {
  ...
  const sourceFiles = await fg(`**/*.{${INPUT_EXTS.join(",")}}`, {
    cwd: CTX.srcRoot,
    absolute: true,
    dot: false,
  });
  ...
  try {
    const menu: MenuManifest = finalManifest.photoDays.map((d: PhotoDay) => {
      ...
      // Get all locations for this day from the final items, maintaining order
      ...
      for (const item of d.items) {
        if (
          item.type === "image" &&
          item.exif?.location &&
          item.exif.location !== "Unknown" &&
          !seenLocations.has(item.exif.location)
        ) {
          locationsInOrder.push(item.exif.location);
          seenLocations.add(item.exif.location);
        } else if (
          item.type === "separator" &&
          item.location &&
          !seenLocations.has(item.location)
        ) {
          locationsInOrder.push(item.location);
          seenLocations.add(item.location);
        }
      }

      const locations = locationsInOrder.map((locationName) => {
        const group = d.items.filter(
          (item) =>
            item.type === "image" && item.exif?.location === locationName,
        );
        const locId =
    ...
    await saveJSON(
      path.join(process.cwd(), "src/lib/menu.manifest.json"),
      menu,
    );
    log.info(
      "Generated lightweight menu manifest at src/lib/menu.manifest.json",
    );
  } catch (e: any) {
...
async function detectChanges(sourceFiles: string[], cache: Cache) {
  ...
  for (const file of sourceFiles) {
    ...
    const outputsExist = await Promise.all(
      cached.outputs.map((p) => fileExists(path.join(CTX.outRoot, p))),
    );
...
async function processImage(absPath: string) {
  ...
  try {
    ...
    const locations = [exif.Sublocation, exif.Location].filter(
      Boolean,
    ) as string[];
    ...
    for (const [variantKey, variantConfig] of Object.entries(config.variants)) {
      for (const format of config.encoding.formats) {
        ...
        const outPath = toPosix(
          path.join(fullFolderName, `${baseName}.${typedFormat}`),
        );
        ...
        if (!ARGS.manifestOnly) {
          ...
          const resizedInstance = sharp(fileBuffer).resize(
    ...
    for (const [outputKey, outputConfig] of Object.entries(
      config.otherOutputs,
    )) {
      const format = "format" in outputConfig ? outputConfig.format : "jpeg";
      const typedFormat = format as "jpeg" | "png";
      const outPath = toPosix(
        path.join(outputConfig.folderName, `${baseName}.${typedFormat}`),
      );
      outputs.push(outPath);
...
async function updateManifest(
  ...
  const manifest: Manifest = await loadJSON(CTX.manifestPath, {
    photoDays: [],
  });
  ...
  manifest.photoDays = manifest.photoDays.filter(
    (day: PhotoDay) => day.items.length > 0,
  );
  ...
  for (const date of Object.keys(resultsByDate)) {
    ...
    let day: PhotoDay | undefined = manifest.photoDays.find(
      (d: PhotoDay) => d.date === date,
    );
  ...
  manifest.photoDays.sort((a: PhotoDay, b: PhotoDay) =>
