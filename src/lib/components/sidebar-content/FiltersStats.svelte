...
<div
  class="relative mb-2 grid grid-cols-4 gap-2 border-b bg-slate-100 px-4 py-3 text-center text-sm text-slate-400 dark:bg-slate-950"
...
>
  ...
  <div>
    ...
    <div class="text-xs text-slate-500 capitalize">{PLURALS.autor(totalAuthors)}</div>
  </div>

  <div>
    <div
      class="text-foreground text-lg font-semibold tracking-tight"
      data-testid="filters-tab-stats-people"
    >
      {totalPeople}
    </div>
    <div class="text-xs text-slate-500 capitalize">{PLURALS.osoba(totalPeople)}</div>
  </div>
