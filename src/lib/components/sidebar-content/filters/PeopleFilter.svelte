  ...
  <Accordion.Content>
    <div class="mb-2 flex w-full justify-between gap-x-2" data-testid="filters-tab-people-control">
      <Button
        variant="outline"
        size="sm"
        title="Zobrazit pouze fotky s lidmi"
    ...
    <div class="grid grid-cols-2 gap-x-12 gap-y-2 pb-4">
      {#each people.displayPersons.filter((p) => p.isUserNamed) as person (person.id)}
        ...
        >
          ...
          <Button
            variant="link"
            size="sm"
            title="Zobrazit pouze tuto osobu"
            class="text-xs"
            onclick={(e) => {
              e.stopPropagation();
      ...
      <p class="col-span-2 pt-1 text-xs text-slate-400">
        Výchozí stav je "Všechno" (zobrazí všechny fotky). Můžete vybrat konkrétní osoby, nebo
        použít filtry nahoře pro rychlý výběr.
      </p>
    </div>
  </Accordion.Content>
</Accordion.Item>
